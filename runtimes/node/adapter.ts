...
const PROVIDERS_ROOT = path.join(CLI_ROOT, "runtimes", "node", "dist", "providers");
...
async function runToolById(toolId, providerId) {
  if (!providerId) {
    throw new Error("provider is required for this runtime tool");
  }

  if (toolId === "node_tool_call_selected_provider") {
    const message = await runModeTest(providerId, "tool_call_test");
    const text =
      (message.content || [])
        ...
        .trim() || "No output.";
    return createTextMessage(
      `Tool: Tool Call Smoke Test\nProvider: ${providerId}\n\n${text}`,
    );
  }

  if (toolId === "node_message_selected_provider") {
    const message = await runModeTest(providerId, "message_test");
    const text =
      (message.content || [])
        .filter((block) => block.type === "text")
        .map((block) => block.text)
...
async function handleMessage(rawData) {
  ...
  if (action === "run_tool") {
    ...
    const message = await runToolById(toolId, providerId);
    return { message };
  }
