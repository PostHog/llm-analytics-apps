...
export class MastraProvider extends BaseProvider {
    ...
    constructor(posthogClient, aiSessionId = null) {
        ...
        const weatherTool = createTool({
            ...
            execute: async ({ context }) => {
                const result = await this.getWeather(context.latitude, context.longitude, context.location_name);
                return { result };
            },
        });
     ...
     */
    captureSpan(spanName, inputState, outputState, latency, spanId, parentId) {
     ...
     */
    captureGeneration(input, output, latency, inputTokens, outputTokens, spanId, parentId) {
        const properties = {
    ...
    async chat(userInput, base64Image) {
        ...
        try {
            ...
            const response = await this.agent.generate(this.messages);
            ...
            if (response.toolCalls && response.toolCalls.length > 0) {
                for (let i = 0; i < response.toolCalls.length; i++) {
                    ...
                    this.captureSpan(toolCall.toolName, toolCall.args, toolResult?.result || toolResult, 0, // Mastra doesn't provide individual tool latency
                    toolSpanId, spanId // Parent is the generation span
                    );
                }
            }
            // Manually capture PostHog generation event
            this.captureGeneration(this.messages.map(m => ({
                role: m.role,
                content: Array.isArray(m.content) ? m.content : [{ type: 'text', text: m.content }]
            })), [{
                    role: 'assistant',
                    content: [{ type: 'text', text: assistantContent }]
                }], latency, inputTokens, outputTokens, spanId);
