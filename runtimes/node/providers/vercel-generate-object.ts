...
export class VercelGenerateObjectProvider extends BaseProvider {
    ...
    getName() {
        return 'Vercel AI SDK - generateObject (OpenAI)';
    }
    // Determine which schema to use based on user input
    determineSchema(userInput) {
    ...
    formatStructuredData(data, type) {
        if (!data)
            return '';
        try {
            if (type === 'weather' && data) {
                ...
                if (data.temperature !== undefined)
                    output += `🌡️  Temperature: ${data.temperature}°C\n`;
                if (data.condition)
                    output += `☁️  Condition: ${data.condition}\n`;
                if (data.humidity !== undefined)
                    output += `💧 Humidity: ${data.humidity}%\n`;
                if (data.windSpeed !== undefined)
                    output += `💨 Wind Speed: ${data.windSpeed} km/h\n`;
            ...
            if (type === 'plan' && data) {
                ...
                if (data.title)
                    output += `📋 ${data.title}\n`;
                if (data.objective)
                    output += `🎯 Objective: ${data.objective}\n`;
                if (data.totalEstimatedTime)
                    output += `⏱️  Total Time: ${data.totalEstimatedTime}\n\n`;
    ...
    // Non-streaming version using generateObject
    async chat(userInput, base64Image) {
        ...
        const model = withTracing(this.openaiClient(modelName), this.posthogClient, {
            ...
            posthogProperties: {
                $ai_span_name: "vercel_ai_generate_object",
        ...
        try {
            ...
            const assistantMessage = {
                role: 'assistant',
                content: `Generated ${type} data: ${JSON.stringify(result.object, null, 2)}`
            };
            this.messages.push(assistantMessage);
            return `✅ Generated structured ${type} data:\n\n${formatted}`;
        }
        catch (error) {
            console.error('Error in Vercel generateObject:', error);
            throw new Error(`Vercel generateObject Provider error: ${error.message}`);
        }
    }
}
//# sourceMappingURL=vercel-generate-object.js.map